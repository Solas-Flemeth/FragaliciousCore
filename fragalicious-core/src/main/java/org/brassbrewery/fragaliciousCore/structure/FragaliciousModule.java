 ...
 */
public abstract class FragaliciousModule<A extends FragaliciousAPI> {
    private final FragaliciousLogger logger;
    private final FragaliciousPlugin plugin;
    private boolean isEnabled = false;
    private A api;

    public FragaliciousModule(FragaliciousPlugin plugin) {
        this.plugin = plugin;
    ...
    public final void enable() {
        ...
        try {
            fine("Starting to launch module");
            preInit();
            registerServices();
            registerListeners();
            registerCommands();
            postInit();
            isEnabled = true;
            fine("Module successfully launched");
        } catch (Throwable t) {
            isEnabled = false;
